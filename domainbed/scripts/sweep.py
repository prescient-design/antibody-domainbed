    ...
    def __init__(self, train_args, sweep_output_dir):
        ...
        for k, v in sorted(self.train_args.items()):
            if isinstance(v, list):
                ...
                raise ValueError(f"{v} not recognized as an argument for {k}.")
...
def make_args_list(n_trials, dataset_names, algorithms, n_hparams_from, n_hparams, steps,
    data_dir, task, holdout_fraction, single_test_envs, hparams, test_envs, path_for_init, target,
    use_esm=None):
    args_list = []
                ...
                for _test_envs in all_test_envs:
                    for hparams_seed in range(n_hparams_from, n_hparams):
                        ...
                        if use_esm is not None:
                            train_args['use_esm'] = None
                        train_args['seed'] = misc.seed_hash(dataset,
                            algorithm, _test_envs, hparams_seed, trial_seed)
                        if steps is not None:
                            train_args['steps'] = steps
                        if hparams is not None:
                            train_args['hparams'] = hparams
                        train_args['target'] = target
...
if __name__ == "__main__":
    ...
    parser.add_argument('--target', type=str, default="IL6")
    parser.add_argument('--test_envs', default=None, nargs="+")
    parser.add_argument('--path_for_init', type=str, default="None")
    parser.add_argument('--use_esm', action='store_true', default=None)
